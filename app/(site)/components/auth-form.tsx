...
const AuthForm = ({ returnPath, callbackError }: AuthFormProps) => {
  ...
  useEffect(() => {
    ...
    setLiveNotice(SIGNED_OUT_NOTICE);
    const frame = requestAnimationFrame(() =>
  ...
  // Set after the card has rendered so it lands as a change in the region
  // that is already mounted. The connection indicator announces "Signed in"
  // at the same moment, so this only says what happens next.
  useEffect(() => {
    if (!currentUser || !authenticatedHere) return;
    setLiveNotice(`Opening your chats, ${displayNameOf(currentUser)}.`);
  }, [authenticatedHere, currentUser]);
  ...
  if (currentUser && authenticatedHere) {
    content = (
      <p style={{ margin: 0, fontSize: 14, color: "var(--t1)" }}>
        {`Signed in as ${displayNameOf(currentUser)}. Opening your chats.`}
      </p>
    );
  } else if (currentUser) {
  ...
  return (
    <div className="gm-glass2" style={cardStyle}>
      {/* One page-local live region for the life of the card, kept apart
          from the connection indicator's status region so the two never
          fight over one announcement. It outlives the branch switch below
          because a region inserted with its text already in place can be
